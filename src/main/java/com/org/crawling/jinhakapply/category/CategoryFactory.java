...
public class CategoryFactory {
    ...
    public static Category create(String categoryName, int categorySeq) {
        ...
        else if (categoryName.matches(CategoryReg.APPLICANTS_COUNT_REG.getReg()))
            return new ApplicantsCount(CategoryReg.APPLICANTS_COUNT_REG.getCategoryName(), categorySeq);
        else if (categoryName.matches(CategoryReg.COMPETITION_RATIO_REG.getReg()))
            return new CompetitionRatio(CategoryReg.COMPETITION_RATIO_REG.getCategoryName(), categorySeq);
        else
    ...
    public static Category createRatio(int rowCount) {
        return new CompetitionRatio(CategoryReg.COMPETITION_RATIO_REG.getCategoryName(), 0, "-1", rowCount);
    }
}
